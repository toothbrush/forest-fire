...
tag: build ## Tag the docker images with the build information.
	...
	docker image tag ${IMAGE_NAME}:latest ${IMAGE_NAME}:${TRAVIS_BRANCH}
	docker image tag ${IMAGE_NAME}:latest ${IMAGE_NAME}:${TRAVIS_BUILD_NUMBER}

push: tag ## Push the docker image to the registry.
	@echo "--- :satellite: Pushing to docker registry :satellite:"
	docker image push ${IMAGE_NAME}:${TRAVIS_BUILD_NUMBER}
	docker image push ${IMAGE_NAME}:${TRAVIS_BRANCH}
...
clean: ## Cleanup artifacts generated by build.
	...
	-docker image rm ${IMAGE_NAME}:${TRAVIS_BRANCH} &> /dev/null
	-docker image rm ${IMAGE_NAME}:${TRAVIS_BUILD_NUMBER} &> /dev/null
